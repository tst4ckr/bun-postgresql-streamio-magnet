...
export class TVAddonConfig {
  ...
  #loadConfiguration() {
    ...
    return {
      // Configuración del addon (valores por defecto para compatibilidad)
      addon: {
        ...
        enableUserConfig: false, // No necesario para generador
        enableDeepLinks: false   // No necesario para generador
      },
      ...
      files: {
        ...
        // Directorio para archivos M3U8 individuales por canal (desde app.conf)
        perChannelM3u8Dir: process.env.PER_CHANNEL_M3U8_DIR || 'data/m3u8'
      },
      ...
      validation: {
        ...
        streamValidationMaxRetries: process.env.STREAM_VALIDATION_MAX_RETRIES !== undefined 
          ? parseInt(process.env.STREAM_VALIDATION_MAX_RETRIES) 
          : 3, // Preserva 0 como valor válido
        ...
        // Normalización de nombres para compatibilidad entre servicios
        // Ambos servicios usan distintas claves (maxValidationRetries, validationMaxRetries)
        // Mapearlas desde la misma variable de entorno STREAM_VALIDATION_MAX_RETRIES
        maxValidationRetries: process.env.STREAM_VALIDATION_MAX_RETRIES
          ? parseInt(process.env.STREAM_VALIDATION_MAX_RETRIES)
          : (parseInt(process.env.MAX_VALIDATION_RETRIES) || 3),
        validationMaxRetries: process.env.STREAM_VALIDATION_MAX_RETRIES
          ? parseInt(process.env.STREAM_VALIDATION_MAX_RETRIES)
          : (parseInt(process.env.VALIDATION_MAX_RETRIES) || 3),
        ...
        httpConversionMaxRetries: process.env.HTTP_CONVERSION_MAX_RETRIES !== undefined 
          ? parseInt(process.env.HTTP_CONVERSION_MAX_RETRIES) 
          : 3, // Preserva 0 como valor válido
        ...
        // Archivos CSV que deben ignorarse en el filtrado de canales prohibidos
        bannedChannelsIgnoreFiles: this.#parseFileList(process.env.BANNED_CHANNELS_IGNORE_FILES),
        ...
        // Configuración de validación de latencia por ping
        enableLatencyValidation: process.env.ENABLE_LATENCY_VALIDATION === 'true',
        maxLatencyMs: parseInt(process.env.MAX_LATENCY_MS) || 50,
        pingTimeoutMs: parseInt(process.env.PING_TIMEOUT_MS) || 3000,
        pingRetries: parseInt(process.env.PING_RETRIES) || 2,
        pingConcurrency: parseInt(process.env.PING_CONCURRENCY) || 10,
        pingCount: parseInt(process.env.PING_COUNT) || 3
      },
