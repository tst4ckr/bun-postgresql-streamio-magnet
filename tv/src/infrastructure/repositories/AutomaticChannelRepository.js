...
export class AutomaticChannelRepository extends ChannelRepository {
  ...
  async #downloadAndProcessM3U() {
    ...
    try {
      ...
      this.#logger.info(`M3U descargado: ${m3uContent.length} bytes`);
      ...
      this.#logger.info(`Playlists generadas: ${playlistUrls.length}`);
      ...
      this.#logger.info(`Canales procesados desde playlists: ${allChannels.length}`);

      // 6. Aplicar filtrado de canales banned y allowed
      const beforeBannedCount = allChannels.length;
      const bannedFilteredChannels = await filterBannedChannels(allChannels);
      const bannedRemovedCount = beforeBannedCount - bannedFilteredChannels.length;
      
      const beforeAllowedCount = bannedFilteredChannels.length;
      const allowedFilteredChannels = filterAllowedChannels(bannedFilteredChannels);
      const allowedRemovedCount = beforeAllowedCount - allowedFilteredChannels.length;
      
      this.#logger.info(`Filtrado aplicado: ${bannedRemovedCount} banned, ${allowedRemovedCount} no-allowed removidos. ${allowedFilteredChannels.length} canales restantes`);

      // 7. Aplicar deduplicación final
      const uniqueChannels = this.#removeDuplicates(allowedFilteredChannels);
      this.#logger.info(`Canales únicos: ${uniqueChannels.length}`);

      // 8. Validar conectividad antes del filtro inteligente (si está habilitado)
      ...
      // 9. Validar conectividad de canales finales después del filtrado (si está habilitado)
      let finalChannels = channelsToFilter;
      if (this.#shouldValidateAfterFiltering()) {
        const postFilterValidation = await this.#validateChannelsAfterFiltering(channelsToFilter);
        finalChannels = postFilterValidation.validChannels;
        this.#logger.info(`Validación post-filtrado: ${postFilterValidation.validChannels.length}/${channelsToFilter.length} válidos`);
      }
      ...
      const batchPromises = batch.map(async (playlistUrl, index) => {
        ...
        try {
          ...
          if (channels.length > 0) {
            this.#logger.debug(`Playlist ${globalIndex} OK: ${channels.length} canales`);
            this.#playlistErrorStats.successfulPlaylists++;
            return channels;
          } else {
            this.#logger.debug(`Playlist ${globalIndex} sin canales válidos`);
            this.#playlistErrorStats.successfulPlaylists++;
            return [];
          }
   ...
   */
  /**
   * Valida conectividad de canales antes del filtrado inteligente
   * Optimizado para logging condicional basado en cantidad de canales
   * @private
   */
  async #validateChannelsBeforeFiltering(channels) {
    ...
    // Siempre log el resultado final
  ...
  async getChannelsPaginated(offset = 0, limit = 50) {
    ...
    const beforeBannedCount = allChannels.length;
    const filteredChannels = await filterBannedChannels(allChannels);
  ...
  async searchChannels(query) {
    ...
    searchResults = await filterBannedChannels(searchResults);
  ...
  async getChannelsByCountry(country) {
    ...
    countryChannels = await filterBannedChannels(countryChannels);
