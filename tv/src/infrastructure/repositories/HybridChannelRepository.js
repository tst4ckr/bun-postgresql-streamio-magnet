...
// Importar servicios de validación de latencia por IP
...
export class HybridChannelRepository extends ChannelRepository {
  ...
  // Servicios de validación de latencia por IP
  #ipExtractionService; // Servicio de extracción de IPs únicas
  #ipLatencyValidationService; // Servicio de validación de latencia por ping
  #memoryOptimizationService; // Servicio de optimización de memoria
   ...
   * @param {ServiceContainer} serviceContainer - Contenedor de servicios (opcional)
   */
  constructor(csvPath, m3uSources, config, logger, serviceContainer = null) {
    super();
    this.#config = config;
    this.#logger = logger;
    
    // Si se proporciona un contenedor de servicios, usarlo para resolver dependencias
    if (serviceContainer) {
      this.#contentFilter = serviceContainer.resolve('contentFilter');
      this.#httpsToHttpService = serviceContainer.resolve('httpsToHttpService');
      this.#streamValidationService = serviceContainer.resolve('streamValidationService');
      this.#ipExtractionService = serviceContainer.resolve('ipExtractionService');
      this.#ipLatencyValidationService = serviceContainer.resolve('ipLatencyValidationService');
      this.#m3uParser = serviceContainer.resolve('m3uParser');
      this.#memoryOptimizationService = serviceContainer.resolve('memoryOptimizationService');
    } else {
      ...
      // Inicializar servicios de validación de latencia por IP
      this.#ipExtractionService = new IpExtractionService(config, logger);
      this.#ipLatencyValidationService = new IpLatencyValidationService(config, logger);
      
      // Inicializar parser para modo automático
      this.#m3uParser = new M3UParserService(config.filters);
      
      // Servicio de optimización de memoria (fallback sin configuración avanzada)
      this.#memoryOptimizationService = null;
    }
  ...
  async #processAutomaticSource() {
    ...
    try {
      ...
      // 6. VALIDACIÓN DE LATENCIA POR IP - Aplicar ANTES de filtros banned/allowed
      let latencyFilteredChannels = allChannels;
      
      if (allChannels.length > 0) {
        // Verificar si la validación de latencia está habilitada
        if (this.#config.validation && this.#config.validation.enableLatencyValidation) {
          this.#logger.info(`🏓 Validando latencia de IPs para ${allChannels.length} canales...`);
          
          try {
            // 1. Extraer IPs únicas de todos los canales
            const ipExtractionResult = await this.#ipExtractionService.extractUniqueIps(allChannels);
            
            if (ipExtractionResult.success && ipExtractionResult.uniqueIps.length > 0) {
              this.#logger.info(`📊 IPs únicas extraídas: ${ipExtractionResult.uniqueIps.length}`);
              
              // 2. Validar latencia de IPs únicas (usando configuración desde .env)
              const latencyValidationResult = await this.#ipLatencyValidationService.validateIpsLatency(
                ipExtractionResult.uniqueIps
              );
              
              if (latencyValidationResult.success) {
                this.#logger.info(`🎯 Validación de latencia completada: ${latencyValidationResult.validIps.length}/${ipExtractionResult.uniqueIps.length} IPs válidas`);
                
                // 3. Filtrar canales basándose en IPs con latencia válida
                const validIpsSet = new Set(latencyValidationResult.validIps);
                latencyFilteredChannels = allChannels.filter(channel => {
                  // Usar la misma lógica de prioridad que en extractUniqueIpsFromChannels
                  let streamUrl = null;
                  if (channel.url) {
                ...
                const extractionResult = this.#ipExtractionService.extractSingleIp(streamUrl);
                return extractionResult.success && validIpsSet.has(extractionResult.ip);
              });
              
              this.#logger.info(`🚀 Canales filtrados por latencia: ${latencyFilteredChannels.length}/${allChannels.length} (${((latencyFilteredChannels.length/allChannels.length)*100).toFixed(1)}%)`);
              
              // Marcar canales con latencia alta como desactivados
              const removedChannels = allChannels.length - latencyFilteredChannels.length;
              if (removedChannels > 0) {
                this.#logger.info(`⚡ ${removedChannels} canales removidos por latencia alta (>50ms)`);
              }
            } else {
              this.#logger.warn('⚠️  Error en validación de latencia, continuando sin filtrado');
              latencyFilteredChannels = allChannels;
            }
          } else {
            this.#logger.warn('⚠️  No se encontraron IPs únicas para validar latencia');
            latencyFilteredChannels = allChannels;
          }
        } catch (error) {
          this.#logger.error(`❌ Error en validación de latencia: ${error.message}`);
          // En caso de error, usar todos los canales sin filtrado
          latencyFilteredChannels = allChannels;
        }
      } else {
        // Validación de latencia deshabilitada - usar todos los canales
        latencyFilteredChannels = allChannels;
        this.#logger.info('⚠️  Validación de latencia deshabilitada - usando todos los canales');
      }
    }

      // 7. Aplicar filtros de contenido (banned y allowed channels) a canales filtrados por latencia
      const bannedFilteredChannels = await filterBannedChannels(latencyFilteredChannels);
      const bannedRemovedCount = latencyFilteredChannels.length - bannedFilteredChannels.length;
      this.#logger.info(`Filtro banned channels: ${bannedRemovedCount} canales removidos`);

      const allowedFilteredChannels = filterAllowedChannels(bannedFilteredChannels);
      const allowedRemovedCount = bannedFilteredChannels.length - allowedFilteredChannels.length;
      this.#logger.info(`Filtro allowed channels: ${allowedRemovedCount} canales removidos`);
      
      const totalFilteredCount = latencyFilteredChannels.length - allowedFilteredChannels.length;
      this.#logger.info(`Total filtrado: ${allowedFilteredChannels.length} canales (${totalFilteredCount} removidos)`);

      // 7. Eliminar duplicados (lógica del modo automático)
      const uniqueChannels = removeDuplicateChannels(allowedFilteredChannels, this.#logger);
  ...
  async initialize() {
    ...
    try {
      ...
      const finalFilteredChannels = await filterBannedChannels(allowedFilteredChannels);
      ...
      // Aplicar optimizaciones de memoria si el servicio está disponible
      if (this.#memoryOptimizationService) {
        try {
          this.#channels = await this.#memoryOptimizationService.optimizeChannelArray(this.#channels);
          this.#logger.info(`🔧 Optimización de memoria aplicada a ${this.#channels.length} canales`);
        } catch (error) {
          this.#logger.warn(`⚠️ Error en optimización de memoria: ${error.message}`);
        }
      }
  ...
  async getAllChannels() {
    ...
    const finalChannels = await filterBannedChannels(activeChannels);
  ...
  async getChannelsByGenre(genre) {
    ...
    const beforeBannedCount = activeChannels.length;
    activeChannels = await filterBannedChannels(activeChannels);
  ...
  async getChannelsByLanguage(language) {
    ...
    // Aplicar filtrado de canales prohibidos
    const finalChannels = await filterBannedChannels(filteredChannels);
