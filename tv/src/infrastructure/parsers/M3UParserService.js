...
export class M3UParserService {
  ...
  /**
   * Procesa entradas en paralelo con fail-fast para errores críticos
   * @private
   */
  async _processEntriesInParallel(rawEntries, config) {
    ...
    for (let i = 0; i < rawEntries.length; i += batchSize) {
      ...
      // Usar Promise.all para detectar errores críticos, fallback a Promise.allSettled
      let batchResults;
      try {
        // Intentar con Promise.all para errores críticos
        const successfulResults = await Promise.all(batchPromises);
        batchResults = successfulResults.map(result => ({ status: 'fulfilled', value: result }));
      } catch (criticalError) {
        // Si hay error crítico en configuración o parsing fundamental, fallar rápido
        if (this._isCriticalParsingError(criticalError)) {
          throw new Error(`❌ Error crítico en parsing M3U: ${criticalError.message}`);
        }
        
        // Para errores no críticos, usar Promise.allSettled como fallback
  ...
  /**
   * Determina si un error de parsing es crítico
   * @private
   */
  _isCriticalParsingError(error) {
    // Errores críticos que deben interrumpir todo el parsing
    const criticalPatterns = [
      /config.*invalid/i,      // Configuración inválida
      /format.*corrupt/i,      // Formato corrupto
      /encoding.*error/i,      // Errores de codificación
      /memory.*exhausted/i,    // Memoria agotada
      /parser.*failed/i        // Fallo del parser
    ];
    
    const errorMessage = error.message || '';
    return criticalPatterns.some(pattern => pattern.test(errorMessage)) ||
           error.code === 'CRITICAL_PARSING_ERROR' ||
