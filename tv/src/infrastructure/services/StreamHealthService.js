...
export class StreamHealthService {
  ...
  constructor(config, logger = console, dependencies = null) {
    ...
    if (dependencies && dependencies.bitelUidService) {
      this.#bitelUidService = dependencies.bitelUidService;
    } else {
   ...
   * @param {Object} options - Opciones adicionales (timeout, maxRetries)
   * @returns {Promise<{ok:boolean,status?:number,contentType?:string,reason?:string}>}
   */
  async checkStream(url, channelId = null, retryCount = 0, options = {}) {
    ...
    const baseTimeout = options.timeout || this.#config.validation.streamValidationTimeout || 45;
    const timeoutMs = calculateProgressiveTimeout(baseTimeout, retryCount);
    const maxRetries = (options.maxRetries !== undefined)
      ? options.maxRetries
      : (this.#config.validation.maxValidationRetries ?? 3);
    ...
    try {
      ...
      if (retryCount < maxRetries && isRetryableError(error)) {
        ...
        return this.checkStream(url, channelId, retryCount + 1, { timeout: options.timeout, maxRetries });
      }
