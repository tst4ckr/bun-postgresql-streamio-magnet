...
export class HttpsToHttpConversionService {
   ...
   * @param {StreamHealthService} streamHealthService - Servicio de salud de streams
   * @param {Object} logger - Logger para trazabilidad
   * @param {Object} dependencies - Dependencias inyectadas (opcional)
   */
  constructor(config, streamHealthService, logger = console, dependencies = null) {
    this.#config = config;
    this.#streamHealthService = streamHealthService;
    this.#logger = logger;
    
    // Si se proporcionan dependencias, usarlas; de lo contrario, crear instancias
    if (dependencies && dependencies.flowControlService) {
      ...
      this.#flowControlService = new ProcessFlowControlService(logger, {
        maxConcurrent: config.httpsToHttp?.maxConcurrentConversions || 5,
        timeout: config.httpsToHttp?.conversionTimeout || 15000
      });
    }
  ...
  async validateHttpUrl(httpUrl) {
    ...
    try {
      const timeoutSec = (this.#config.validation?.httpConversionTimeout ?? this.#config.validation?.streamValidationTimeout) || 20;
      const maxRetries = (this.#config.validation?.httpConversionMaxRetries ?? this.#config.validation?.maxValidationRetries ?? this.#config.validation?.streamValidationMaxRetries) ?? 3;
      const result = await this.#streamHealthService.checkStream(httpUrl, null, 0, {
        timeout: timeoutSec,
        maxRetries
      });
      return result;
