...
export function channelToCSVRecord(channel) {
  return {
    ...
    logo: channel.logo || '',
    // Tomar poster/background desde propiedades del canal o metadatos originales
    poster: (channel.poster || channel?.metadata?.poster || channel?.metadata?.originalData?.poster || ''),
    background: (channel.background || channel?.metadata?.background || channel?.metadata?.originalData?.background || ''),
